...
// }
SRTF(1, 5, [0, 2, 1, 4, 3], [8, 6, 4, 3, 2], 1)
/*TEST FINISH*/

function SRTF(clickFrequency, task_num, commit_time, task_time, time_block)
{
    var i, j;
    var flag = 0; //flag为0意为作业未全部完成，而当所有works的finish_time全不为""时，则将flag置为1，SRTF算法结束
    var temp = [0, 0]; //临时变量，用于存储当前剩余时间最短的作业的作业号和时间
    var works = [];
    var temp_1; //用于临时存储剩余时间
    ...
    {
        fmtrst.push({
            startTime_1: works[j].startTime_1 === "" ? "": parseFloat(works[j].startTime_1.toFixed(2)),
            startTime_2: works[j].startTime_2 === "" ? "": parseFloat(works[j].startTime_2.toFixed(2)),
            startTime_3: works[j].startTime_3 === "" ? "": parseFloat(works[j].startTime_3.toFixed(2)),
            finishTime: works[j].finish_time === "" ? "": parseFloat(works[j].finish_time.toFixed(2)),
            turnOverTime: works[j].turnOverTime === "" ? "": parseFloat(works[j].turnOverTime.toFixed(2)),
            weightedTurnOverTime: works[j].weightedTurnOverTime === "" ? "": parseFloat(works[j].weightedTurnOverTime.toFixed(2)),
            remainTime: works[j].remain_time === "" ? "": parseFloat(works[j].remain_time.toFixed(2))
          })
    }
    running_log.push({
        formatResult: fmtrst.slice(0),
    ...
    //开始SRTF计算
    for (i=0; flag==0; i+=time_block) //每次循环模拟系统运行的一个单位时间
    {
        ...
        if (temp[0] !== "")
        {
            ...
            temp_1 = works[temp[0]].remain_time;
            works[temp[0]].remain_time = works[temp[0]].remain_time - time_block; //该作业剩余时间减1
            if (works[temp[0]].remain_time <= 0.0001)
            {
                works[temp[0]].remain_time = 0; //如果运行单位时间后剩余时间小于零，说明运行前剩余时间小于单位时间，则将剩余时间置为零
                works[temp[0]].finish_time = i+temp_1; //如果该作业运行完这一单位时间后剩余时间为0，则将当前之间加上次的剩余时间设为该作业的结束时间
            ...
            running_log.push({
                ...
                currentTime:  parseFloat((i + time_block).toFixed(2)).toString()
            });
        }
